...
static int fts_pram_ecc_cal_algo(struct i2c_client *client, u32 start_addr, u32 ecc_length)
{
	...
	MI_TOUCH_LOGI(1, "read out pramboot checksum");
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "write pramboot ecc cal cmd fail");
		return ret;
	}
	...
	for (i = 0; i < 100; i++) {
		...
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "ecc_finish read cmd fail");
			return ret;
		}
		if (0 == val[0])
			break;
	...
	if (i >= 100) {
		MI_TOUCH_LOGE(1, "wait ecc finish fail");
		return -EIO;
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "read pramboot ecc fail");
		return ret;
	}
...
static int fts_pram_ecc_cal(struct i2c_client *client, u32 saddr, u32 len)
{
	if ((NULL == fwupgrade) && (NULL == fwupgrade->func)) {
		MI_TOUCH_LOGE(1, "fwupgrade/func is null");
		return -EINVAL;
	}
...
static int fts_pram_write_buf(struct i2c_client *client, u8 *buf, u32 len)
{
	...
	MI_TOUCH_LOGI(1, "write pramboot to pram");
	if ((NULL == fwupgrade) && (NULL == fwupgrade->func)) {
		MI_TOUCH_LOGE(1, "fwupgrade/func is null");
		return -EINVAL;
	}
	...
	if (NULL == buf) {
		MI_TOUCH_LOGE(1, "pramboot buf is null");
		return -EINVAL;
	}

	MI_TOUCH_LOGI(1, "pramboot len=%d", len);
	if ((len < PRAMBOOT_MIN_SIZE) || (len > PRAMBOOT_MAX_SIZE)) {
		MI_TOUCH_LOGE(1, "pramboot length(%d) fail", len);
		return -EINVAL;
	}
	...
	for (i = 0; i < packet_number; i++) {
		...
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "pramboot write data(%d) fail", i);
			return ret;
		}
	}
...
static int fts_pram_start(struct i2c_client *client)
{
	...
	MI_TOUCH_LOGI(1, "remap to start pramboot");

	ret = fts_i2c_write(client, &cmd, 1);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "write start pram cmd fail");
		return ret;
	}
...
static int fts_pram_write_remap(struct i2c_client *client)
{
	...
	MI_TOUCH_LOGI(1, "write pram and remap");

	if (!upg || !upg->func || !upg->func->pramboot) {
		MI_TOUCH_LOGE(1, "upgrade/pramboot is null");
		return -EINVAL;
	}

	if (upg->func->pb_length < FTS_MIN_LEN) {
		MI_TOUCH_LOGE(1, "pramboot length(%x) fail", upg->func->pb_length);
		return -EINVAL;
	}
	...
	if (ecc_in_host < 0) {
		MI_TOUCH_LOGE(1, "write pramboot fail");
		return ecc_in_host;
	}
	...
	MI_TOUCH_LOGI(1, "pram ecc in tp:%x, host:%x", ecc_in_tp, ecc_in_host);
	/*  pramboot checksum != fw checksum, upgrade fail */
	if (ecc_in_host != ecc_in_tp) {
		MI_TOUCH_LOGE(1, "pramboot ecc check fail");
		return -EIO;
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "pram start fail");
		return ret;
	}
...
static int fts_pram_init(struct i2c_client *client)
{
	...
	MI_TOUCH_LOGI(1, "pramboot initialization");

	/* read flash ID */
	wbuf[0] = FTS_CMD_FLASH_TYPE;
	ret = fts_i2c_read(client, wbuf, 1, &reg_val, 1);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "read flash type fail");
		return ret;
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "write flash type fail");
		return ret;
	}
...
int fts_pram_write_init(struct i2c_client *client)
{
	...
	MI_TOUCH_LOGI(1, "**********pram write and init**********");
	if ((NULL == upg) || (NULL == upg->func)) {
		MI_TOUCH_LOGE(1, "upgrade/func is null");
		return -EINVAL;
	}

	if (!upg->func->pramboot_supported) {
		MI_TOUCH_LOGE(1, "ic not support pram");
		return -EINVAL;
	}

	MI_TOUCH_LOGN(1, "check whether tp is in romboot or not ");
	...
	if (status != FTS_RUN_IN_ROM) {
		if (FTS_RUN_IN_PRAM == status) {
			MI_TOUCH_LOGI(1, "tp is in pramboot, need send reset cmd before upgrade");
			ret = fts_pram_init(client);
			if (ret < 0) {
				MI_TOUCH_LOGE(1, "pramboot(before) init fail");
				return ret;
			}
		}

		MI_TOUCH_LOGI(1, "tp isn't in romboot, need send reset to romboot");
		ret = fts_fwupg_reset_to_romboot(client);
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "reset to romboot fail");
			return ret;
		}
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "pram write fail, ret=%d", ret);
		return ret;
	}

	MI_TOUCH_LOGN(1, "after write pramboot, confirm run in pramboot");
	state = fts_fwupg_check_state(client, FTS_RUN_IN_PRAM);
	if (!state) {
		MI_TOUCH_LOGE(1, "not in pramboot");
		return -EIO;
	}

	ret = fts_pram_init(client);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "pramboot init fail");
		return ret;
	}
...
bool fts_fwupg_check_fw_valid(struct i2c_client *client)
{
	...
	if (ret < 0) {
		MI_TOUCH_LOGI(1, "tp fw invaild");
		return false;
	}

	MI_TOUCH_LOGI(1, "tp fw vaild");
	return true;
}
...
int fts_fwupg_get_boot_state(struct i2c_client *client, enum FW_STATUS *fw_sts)
{
	...
	MI_TOUCH_LOGI(1, "**********read boot id**********");
	if ((NULL == fw_sts) || (NULL == upg) || (NULL == upg->func)) {
		MI_TOUCH_LOGE(1, "upgrade/func/fw_sts is null");
		return -EINVAL;
	}
...
int fts_fwupg_reset_in_boot(struct i2c_client *client)
{
	...
	MI_TOUCH_LOGI(1, "reset in boot environment");
	ret = fts_i2c_write(client, &cmd, 1);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "pram/rom/bootloader reset cmd write fail");
		return ret;
	}
...
int fts_fwupg_reset_to_boot(struct i2c_client *client)
{
	...
	MI_TOUCH_LOGI(1, "send 0xAA and 0x55 to FW, reset to boot environment");

	ret = fts_i2c_write_reg(client, FTS_REG_UPGRADE, FTS_UPGRADE_AA);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "write FC=0xAA fail");
		return ret;
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "write FC=0x55 fail");
		return ret;
	}
...
int fts_fwupg_enter_into_boot(struct i2c_client *client)
{
	...
	MI_TOUCH_LOGI(1, "***********enter into pramboot/bootloader***********");
	if ((NULL == upg) || (NULL == upg->func)) {
		MI_TOUCH_LOGE(1, "upgrade/func is null");
		return -EINVAL;
	}
	...
	if (fts_data->fw_forceupdate)
		...
		if (fwvalid) {
			...
			if (ret < 0) {
				MI_TOUCH_LOGE(1, "enter into romboot/bootloader fail");
				return ret;
			}
		}
	}
	...
	if (upg->func->pramboot_supported) {
		MI_TOUCH_LOGI(1, "pram supported, write pramboot and init");
		/* pramboot */
		ret = fts_pram_write_init(client);
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "pram write_init fail");
			return ret;
		}
	} else {
		MI_TOUCH_LOGN(1, "pram not supported, confirm in bootloader");
		/* bootloader */
		state = fts_fwupg_check_state(client, FTS_RUN_IN_BOOTLOADER);
		if (!state) {
			MI_TOUCH_LOGE(1, "fw not in bootloader, fail");
			return -EIO;
		}
	}
...
static bool fts_fwupg_check_flash_status(struct i2c_client *client, u16 flash_status, int retries, int retries_delay)
{
	...
	for (i = 0; i < retries; i++) {
		...
		if (flash_status == read_status) {
			/* MI_TOUCH_LOGN(1, "[UPGRADE]flash status ok"); */
			return true;
		}
		/* MI_TOUCH_LOGN(1, "flash status fail,ok:%04x read:%04x, retries:%d", flash_status, read_status, i); */
...
int fts_fwupg_erase(struct i2c_client *client, u32 delay)
{
	...
	MI_TOUCH_LOGI(1, "**********erase now**********");

	/*send to erase flash */
	cmd = FTS_CMD_ERASE_APP;
	ret = fts_i2c_write(client, &cmd, 1);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "erase cmd fail");
		return ret;
	}
	msleep(delay);
	...
	if (!flag) {
		MI_TOUCH_LOGE(1, "ecc flash status check fail");
		return -EIO;
	}
...
int fts_fwupg_ecc_cal(struct i2c_client *client, u32 saddr, u32 len)
{
	...
	MI_TOUCH_LOGI(1, "**********read out checksum**********");

	/* check sum init */
	wbuf[0] = FTS_CMD_ECC_INIT;
	ret = fts_i2c_write(client, wbuf, 1);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "ecc init cmd write fail");
		return ret;
	}
	...
	MI_TOUCH_LOGI(1, "ecc calc num:%d, remainder:%d", packet_num, remainder);
	...
	for (i = 0; i < packet_num; i++) {
		...
		MI_TOUCH_LOGN(1, "ecc calc startaddr:0x%04x, len:%d", addr, packet_len);
		ret = fts_i2c_write(client, wbuf, FTS_CMD_ECC_CAL_LEN);
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "ecc calc cmd write fail");
			return ret;
		}
		...
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "ecc flash status read fail");
			return ret;
		}
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "ecc read cmd write fail");
		return ret;
	}
...
int fts_flash_write_buf(struct i2c_client *client, u32 saddr, u8 *buf, u32 len, u32 delay)
{
	...
	MI_TOUCH_LOGI(1, "**********write data to flash**********");

	if ((NULL == buf) || (0 == len)) {
		MI_TOUCH_LOGE(1, "buf is NULL or len is 0");
		return -EINVAL;
	}

	MI_TOUCH_LOGI(1, "data buf start addr=0x%x, len=0x%x", saddr, len);
	packet_number = len / FTS_FLASH_PACKET_LENGTH;
	remainder = len % FTS_FLASH_PACKET_LENGTH;
	if (remainder > 0)
		packet_number++;
	packet_len = FTS_FLASH_PACKET_LENGTH;
	MI_TOUCH_LOGI(1, "write data, num:%d remainder:%d", packet_number, remainder);
	...
	for (i = 0; i < packet_number; i++) {
		...
		ret = fts_i2c_write(client, packet_buf, packet_len + FTS_CMD_WRITE_LEN);
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "app write fail");
			return ret;
		}
		mdelay(delay);
		...
		for (j = 0; j < FTS_RETRIES_WRITE; j++) {
			...
			ret = fts_i2c_read(client, &cmd, 1, val, FTS_CMD_FLASH_STATUS_LEN);
			read_status = (((u16) val[0]) << 8) + val[1];
			/*  MI_TOUCH_LOGI(1, "%x %x", wr_ok, read_status); */
...
int fts_flash_read_buf(struct i2c_client *client, u32 saddr, u8 *buf, u32 len)
{
	...
	packet_len = FTS_FLASH_PACKET_LENGTH;
	MI_TOUCH_LOGI(1, "read packet_number:%d, remainder:%d", packet_number, remainder);
	...
	for (i = 0; i < packet_number; i++) {
		...
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "pram/bootloader write 03 command fail");
			return ret;
		}
		...
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "pram/bootloader read 03 command fail");
			return ret;
		}
	}
...
int fts_flash_read(struct i2c_client *client, u32 addr, u8 *buf, u32 len)
{
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "enter into pramboot/bootloader fail");
		goto read_flash_err;
	}

	ret = fts_flash_read_buf(client, addr, buf, len);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "read flash fail");
		goto read_flash_err;
	}
...
int fts_upgrade_bin(struct i2c_client *client, char *fw_name, bool force)
{
	...
	MI_TOUCH_LOGI(1, "start upgrade with fw bin");
	if ((NULL == upg) || (NULL == upg->func)) {
		MI_TOUCH_LOGE(1, "upgrade/func is null");
		return -EINVAL;
	}
	if (!fw_name) {
		MI_TOUCH_LOGE(1, "fw name is null");
		return -EINVAL;
	}
	...
	if (ret == 0) {
		...
		if (fw_file_buf == NULL) {
			MI_TOUCH_LOGE(1, "alloc fw buffer error");
			release_firmware(fw);
			return -EINVAL;
		}
		...
		MI_TOUCH_LOGE(1, "request fw error");
		return -EINVAL;
	}

	MI_TOUCH_LOGI(1, "fw bin file len:%x", fw_file_len);
	if (force) {
		if (upg->func->force_upgrade) {
			ret = upg->func->force_upgrade(client, fw_file_buf, fw_file_len);
		} else {
			MI_TOUCH_LOGI(1, "force_upgrade function is null, no upgrade");
			goto err_bin;
		}
	} else {
		if (upg->func->upgrade) {
			...
			MI_TOUCH_LOGI(1, "upgrade function is null, no upgrade");
		}
	}

	if (ret < 0) {
		MI_TOUCH_LOGE(1, "upgrade fw bin failed");
		fts_fwupg_reset_in_boot(client);
		goto err_bin;
	}

	MI_TOUCH_LOGI(1, "upgrade fw bin success");
...
static int fts_param_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
	...
	if (NULL == ver) {
		MI_TOUCH_LOGE(1, "ver is NULL");
		return -EINVAL;
	}

	ret = fts_i2c_read_reg(client, FTS_REG_IDE_PARA_VER_ID, ver);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "read fw param ver from tp fail");
		return ret;
	}

	if ((0x00 == *ver) || (0xFF == *ver)) {
		MI_TOUCH_LOGI(1, "param version in tp invalid");
		return -EIO;
	}
...
static int fts_param_get_ver_in_host(u8 *ver)
{
	...
	if ((!upg) || (!upg->func) || (!upg->fw) || (!ver)) {
		MI_TOUCH_LOGE(1, "fts_data/upgrade/func/fw/ver is NULL");
		return -EINVAL;
	}

	if (upg->fw_length < upg->func->paramcfgveroff) {
		MI_TOUCH_LOGE(1, "fw len(%x) < paramcfg ver offset(%x)", upg->fw_length, upg->func->paramcfgveroff);
		return -EINVAL;
	}

	MI_TOUCH_LOGI(1, "fw paramcfg version offset:%x", upg->func->paramcfgveroff);
	*ver = upg->fw[upg->func->paramcfgveroff];

	if ((0x00 == *ver) || (0xFF == *ver)) {
		MI_TOUCH_LOGI(1, "param version in host invalid");
		return -EIO;
	}
...
static bool fts_param_need_upgrade(struct i2c_client *client)
{
	...
	fwvalid = fts_fwupg_check_fw_valid(client);
	if (!fwvalid) {
		MI_TOUCH_LOGI(1, "fw is invalid, no upgrade paramcfg");
		return false;
	}

	ret = fts_param_get_ver_in_host(&ver_in_host);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "param version in host invalid");
		return false;
	}

	ret = fts_i2c_read_reg(client, FTS_REG_IDE_PARA_STATUS, &val);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "read IDE PARAM STATUS in tp fail");
		return false;
	}

	if ((val & 0x80) != 0x80) {
		MI_TOUCH_LOGI(1, "no IDE VER in tp");
		return false;
	} else if ((val & 0x7F) != 0x00) {
		MI_TOUCH_LOGI(1, "IDE VER, param invalid, need upgrade param");
		return true;
	}

	ret = fts_param_get_ver_in_tp(client, &ver_in_tp);
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "get IDE param ver in tp fail");
		return false;
	}

	MI_TOUCH_LOGI(1, "fw paramcfg version in tp:%x, host:%x", ver_in_tp, ver_in_host);
...
static int fts_fwupg_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "read fw ver from tp fail");
		return ret;
	}
...
static int fts_fwupg_get_ver_in_host(u8 *ver)
{
	...
	if (upg->fw_length < upg->func->fwveroff) {
		MI_TOUCH_LOGE(1, "fw len(%x) < fw ver offset(%x)", upg->fw_length, upg->func->fwveroff);
		return -EINVAL;
	}

	MI_TOUCH_LOGI(1, "fw version offset:0x%x", upg->func->fwveroff);
...
static bool fts_fwupg_need_upgrade(struct i2c_client *client)
{
	...
	if (!ts_data) {
		MI_TOUCH_LOGE(1, "can't get ts data");
		return false;
	}
	...
	if (fwvalid) {
		...
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "get fw ver in host fail");
			return false;
		}

		ret = fts_fwupg_get_ver_in_tp(client, &fw_ver_in_tp);
		if (ret < 0) {
			MI_TOUCH_LOGE(1, "get fw ver in tp fail");
			return false;
		}

		MI_TOUCH_LOGI(1, "fw version in tp:0x%x, host:0x%x", fw_ver_in_tp, fw_ver_in_host);
		if (fw_ver_in_tp != fw_ver_in_host) {
			return true;
		}
	} else {
		MI_TOUCH_LOGI(1, "fw invalid, need upgrade fw");
		return true;
	}
...
int fts_fwupg_upgrade(struct i2c_client *client, struct fts_upgrade *upg)
{
	...
	MI_TOUCH_LOGI(1, "fw auto upgrade function");
	if ((NULL == upg) || (NULL == upg->func)) {
		MI_TOUCH_LOGE(1, "upg/upg->func is null");
		return -EINVAL;
	}
	...
	MI_TOUCH_LOGI(1, "fw upgrade flag:%d", upgrade_flag);
	do {
		upgrade_count++;
		if (upgrade_flag) {
			MI_TOUCH_LOGI(1, "upgrade fw app(times:%d)", upgrade_count);
			if (upg->func->upgrade) {
				...
				if (ret < 0) {
					...
					MI_TOUCH_LOGI(1, "success upgrade to fw version 0x%02x", ver);
					break;
				}
			} else {
				MI_TOUCH_LOGE(1, "upgrade func/upgrade is null, return immediately");
				ret = -ENODATA;
				break;
			}
		} else {
			MI_TOUCH_LOGI(1, "fw don't need upgrade!");
			if (upg->func->param_upgrade) {
				if (fts_param_need_upgrade(client)) {
					MI_TOUCH_LOGI(1, "upgrade param area(times:%d)", upgrade_count);
					ret = upg->func->param_upgrade(client, upg->fw, upg->fw_length);
					if (ret < 0) {
						fts_fwupg_reset_in_boot(client);
					} else {
						fts_param_get_ver_in_tp(client, &ver);
						MI_TOUCH_LOGI(1, "success upgrade to fw param version 0x%02x", ver);
						break;
					}
				} else {
					MI_TOUCH_LOGI(1, "param don't need upgrade");
					break;
				}
			} else {
				break;
...
void fts_fwupg_auto_upgrade(struct fts_ts_data *ts_data)
{
	...
	MI_TOUCH_LOGI(1, "********************FTS enter upgrade********************");

	ret = fts_fwupg_upgrade(client, upg);
	if (ret < 0)
		MI_TOUCH_LOGE(1, "**********tp fw(app/param) upgrade failed**********");
	else
		MI_TOUCH_LOGE(1, "**********tp fw(app/param) upgrade success*********");

	MI_TOUCH_LOGI(1, "********************FTS exit upgrade********************");
}
...
int fts_fwupg_get_vendorid(struct fts_ts_data *ts_data, u16 *vid)
{
	...
	MI_TOUCH_LOGI(1, "read vendor id from tp");
	if ((!upg) || (!upg->func) || (!vid)) {
		MI_TOUCH_LOGE(1, "upgrade/func/vid is null");
		return -EINVAL;
	}
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "fail to get vendor id from tp");
		return ret;
	}
...
static int fts_fwupg_get_fw_file(struct fts_ts_data *ts_data)
{
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "get vendor id failed");
		return ret;
	}
	MI_TOUCH_LOGI(1, "success to read vendor id:%04x", vendor_id);
	for (i = 0; i < FTS_GET_VENDOR_ID_NUM; i++) {
		fw = &fw_list[i];
		if (vendor_id == fw->vendor_id) {
			MI_TOUCH_LOGI(1, "vendor id match, get fw file successfully");
			break;
		}
	}
	if (i >= FTS_GET_VENDOR_ID_NUM) {
		MI_TOUCH_LOGE(1, "no vendor id match, don't get file");
		return -ENODATA;
	}
#endif
	...
	if (upg) {
		...
		MI_TOUCH_LOGI(1, "upgrade fw file len:0x%x", upg->fw_length);
		if ((upg->fw_length < FTS_MIN_LEN)
		    || (upg->fw_length > FTS_MAX_LEN_FILE)) {
			MI_TOUCH_LOGE(1, "fw file len(%x) fail", upg->fw_length);
			return -ENODATA;
		}
	}
	return 0;
...
static void fts_fwupg_work(struct work_struct *work)
{
	...
	MI_TOUCH_LOGI(1, "fw upgrade work function");
	...
	MI_TOUCH_LOGI(1, "get upgrade fw file");
	ret = fts_fwupg_get_fw_file(ts_data);
	fts_fwupg_init_ic_detail();
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "get file fail, can't upgrade");
	} else {
		/* run auto upgrade */
...
static int fts_get_lockdown_info(struct fts_ts_data *ts_data)
{
	...
	if (ret < 0) {
		MI_TOUCH_LOGE(1, "fail to get lockdown info");
		return ret;
	}

	for (i = 0; i < FTS_LOCKDOWN_INFO_SIZE; i++)
		MI_TOUCH_LOGI(1, "Lockdown info[%d] = 0x%02x", i, ts_data->lockdown_info[i]);
	...
	MI_TOUCH_LOGI(1, "Lockdown info = %s", buf);
	return ret;
}
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
	...
	MI_TOUCH_LOGI(1, "fw upgrade init function");

	if (NULL == ts_data->ts_workqueue) {
		MI_TOUCH_LOGE(1, "fts workqueue is NULL, can't run upgrade function");
		return -EINVAL;
	}

	if (0 == func_count) {
		MI_TOUCH_LOGE(1, "no upgrade function in tp driver");
		return -ENODATA;
	}

	fwupgrade = (struct fts_upgrade *)kzalloc(sizeof(*fwupgrade), GFP_KERNEL);
	if (NULL == fwupgrade) {
		MI_TOUCH_LOGE(1, "malloc memory for upgrade fail");
		return -ENOMEM;
	}
		...
		for (i = 0; i < func_count; i++) {
			...
			for (j = 0; j < FTX_MAX_COMPATIBLE_TYPE; j++) {
				if (0 == func->ctype[j])
					...
					MI_TOUCH_LOGI(1, "match upgrade function,type:%x", (int)func->ctype[j]);
					fwupgrade->func = func;
				}
			}
		}
	...
	if (NULL == fwupgrade->func) {
		MI_TOUCH_LOGE(1, "no upgrade functin match, can't upgrade");
		return -ENODATA;
	}

	ret = fts_get_lockdown_info(ts_data);
	if (ret) {
		MI_TOUCH_LOGE(1, "fail get lockdown info");
	}
